...
public class CacheScoreComputer extends AbstractAlgorithme
{
    ...
    public SortieAlgorithme optimiser(FileData entree)
    {
        ...
        for (Entry<Endpoint, List<Request>> entry : mapEndpointsRequests.entrySet())
        {
            ...
            for (Entry<Integer, Cache> entryCache : enpoint.getCaches().entrySet())
            {
                associateRequestToCache(entryCache.getValue(), enpoint.getCachesLatencies().get(entryCache.getKey()),
                    enpoint.getDatacenterLatency(), entry.getValue(), mapCacheRequests);
            }
        }

        SortieAlgorithme resultat = new SortieAlgorithme();
        List<CacheSortie> listCacheSortie = new ArrayList<CacheSortie>();
        ...
        for (Entry<Cache, List<VideoInCache>> entryVideo : mapCacheRequests.entrySet())
        {
            ...
            List<Integer> listVid = calculterListePourCache(cache, entryVideo.getValue());
    ...
    protected void associateRequestToCache(Cache cache, Integer latenceCache, int latenceDataCenter, List<Request> listRequetes,
        Map<Cache, List<VideoInCache>> mapCacheVideo)
    {
        ...
        // TODO : la vidéo peut être déjà présente....
        for (Request req : listRequetes)
        {
            ...
            vid.setLatencyDataCenter(latenceDataCenter);
            listVid.add(vid);
        }
    }
    ...
    protected List<Integer> calculterListePourCache(Cache cache, List<VideoInCache> listVideosInCache)
    {
        ...
        {
            if (tailleUtiliseDansCache + video.getVideo().getSize() < cache.getSize())
            {
    ...
    {
//        new CacheScoreComputer().run("D:/kittens.in");
//        new CacheScoreComputer().run("D:/me_at_the_zoo.in");
        new CacheScoreComputer().run("D:/exemple.in");
    }
}
